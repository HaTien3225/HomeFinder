...
const RoomDetails = ({ route }) => {
    ...
    const [comments, setComments] = React.useState([]);  // Khởi tạo state cho bình luận

    // Hàm tải chi tiết phòng trọ từ API
    const loadRoom = async () => {
        try {
            let res = await APIs.get(endpoints['roomRequest'](roomId));  // Gọi API với roomId
            setRoom(res.data);
        } catch (error) {
    ...
    const reachBottom = ({ nativeEvent }) => {
        if (isCloseToBottom(nativeEvent) && comments.length > 0) {
    ...
    return (
        <ScrollView onScroll={reachBottom}>
            {room === null ? (
                ...
                <Card>
                    <Card.Cover source={{ uri: room.images }} /> {/* Dùng images thay vì image */}
                    <Card.Content>
                        <Text variant="titleLarge" style={MyStyles.roomTitle}>{room.title}</Text> {/* Thay name bằng title */}
                        ...
                        <RenderHTML source={{ html: room.description }} /> {/* Đảm bảo nội dung HTML */}
                    </Card.Content>
                </Card>
            )}
                ...
                {comments.length === 0 ? (
                    ...
                    comments.map((c) => (
                        <List.Item
                            ...
                            left={() => (
                                <Image style={MyStyles.box} source={{ uri: c.user.avatar }} />  
                            )}
                        />
                    ))
                )}
